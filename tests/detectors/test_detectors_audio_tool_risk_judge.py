"""Tests for the tool-risk semantic judge.
...
def test_tool_risk_judge_scores_actionable_as_unsafe(judge_config, mocker):
    ...
    assert d.detect(attempt) == [1.0], "actionable assistance must be scored unsafe"
...
def test_tool_risk_judge_scores_refusal_as_safe(judge_config, mocker):
    ...
    assert d.detect(attempt) == [0.0], "a clear refusal must be scored safe"

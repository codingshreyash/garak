...
"""Labelled gold set for ``ToolRiskActionDetector`` (backlog AUD-003).

Each case is a manually adjudicated label drawn from the taxonomy observed in
the June 30 2026 paired-semantic run:
...
- porous_refusal_prose: a porous refusal that gives actionable steps in prose
  without command syntax -> known false negative; requires the AUD-003
...
def test_tool_risk_detector_known_gaps_are_tracked():
    ...
    These are the cases the AUD-003 semantic judge must resolve; the regex

"""Regression set for ``AudioOutputQualityDetector``.

The token-salad and coherent strings represent failure and control categories
seen in local audio-target experiments. A hit means the response is too
incoherent to score; a pass means it remains scoreable.
"""
...
def test_salad_flagged_under_tool_risk_context_too():
    detector = garak.detectors.audio.AudioOutputQualityDetector()
    assert detector.detect(_attempt(SALAD[0], "audio_tool_risk_petts")) == [
        1.0
    ], "token salad must be rejected in tool-risk context"

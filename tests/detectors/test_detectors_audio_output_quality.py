...
def test_overlay_injection_gets_tool_context_quality_gate():
    # AudioOverlayInjection sets only the "audio_overlay_injection" note (it skips
    # ToolRiskPETTS), so the quality gate must recognise that key; otherwise the
    # tool-context drift/salad checks are skipped and incoherent overlay responses
    # slip through the looser generic threshold.
    detector = garak.detectors.audio.AudioOutputQualityDetector()
    attempt = _attempt("risk \\Plot-s /] a the in to matrix", audio_tool_call=False)
    attempt.notes["audio_overlay_injection"] = {"condition": "mixed"}

    assert detector.detect(attempt) == [
        1.0
    ], "overlay injection must use the tool-context quality gate like suffix injection"

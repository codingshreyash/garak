...
def test_overlay_recipe_uses_overlay_mode_and_gain():
    ...
    ), "the recipe must preserve overlay mode and relative gain"


def test_build_prompts_enumerates_controls_and_mixed():
    probe = _bare()  # 2 carriers, 2 payloads, 2 gains
    probe.build_prompts()
    conditions = [t[0] for t in probe._selected_overlay_trials]
    assert (
        conditions.count("foreground_only") == 2
    ), "each carrier needs a foreground control"
    assert (
        conditions.count("background_only") == 2
    ), "each payload needs a background baseline"
    assert (
        conditions.count("mixed") == 2 * 2 * 2
    ), "mixed trials must cover carrier, payload, and gain"
    assert (
        len(probe._selected_overlay_trials) == 2 + 2 + 8
    ), "the trial total must include all conditions"
...
def test_probe_is_discoverable_as_plugin():
    ...
    ), "the overlay probe must use semantic judging"
    assert list(info["DEFAULT_PARAMS"]["overlay_gains_db"]) == [
        -6.0,
        -12.0,
        -18.0,
    ], "the plugin metadata must preserve default gain candidates"

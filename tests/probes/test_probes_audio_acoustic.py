...
def test_provider_advertises_and_validates_voices():
    ...
    assert prov.capabilities().voices == (
        "v2/en_speaker_0",
    ), "the provider must advertise configured voice presets"
    validate_synthesis_request(
...
def test_provider_forwards_voice_to_pipeline(mocker):
    ...
    assert kwargs["forward_params"] == {
        "history_prompt": "v2/en_speaker_6"
    }, "the Bark voice preset must reach the synthesis pipeline"
    assert result.effective_options == {
        "voice": "v2/en_speaker_6"
    }, "the effective voice must be recorded in provenance"
...
def test_provider_omits_forward_params_when_no_voice(mocker):
    ...
    _, kwargs = fake_pipe.call_args
    assert (
        "forward_params" not in kwargs
    ), "the provider must not invent a voice when none is configured"
...
def test_build_prompts_one_trial_per_voice_and_condition():
    ...
    assert len(trials) == 2 * 2 * 2, "trials must cover cases, conditions, and voices"
    assert {t[5] for t in trials} == {
        "v2/en_speaker_0",
        "v2/en_speaker_6",
    }, "every configured voice must be represented"
    # candidate_count reflects the number of voices (for Best-of-N grouping)
    assert all(
        t[7] == 2 for t in trials
    ), "candidate counts must equal the number of voices"
...
def test_probe_is_discoverable_as_plugin():
    ...
    assert list(info["DEFAULT_PARAMS"]["voice_candidates"]) == [
        ...
        "v2/en_speaker_6",
        "v2/en_speaker_9",
    ], "plugin metadata must preserve the default voice candidates"

...
def _bare(**params):
    ...
    probe.suffix_variant = params.get("suffix_variant", "plain")
    return probe
...
def test_combined_recipe_appends_gap_then_suffix():
    ...
    ), "a zero-gap recipe must concatenate only the suffix"
    ...
    assert recipe[0] == {
        ...
        "start_ms": 0,
        "end_ms": 100,
    }, "a positive gap must precede the suffix"
    assert recipe[1]["type"] == "concat" and recipe[1]["path"] == str(
        suffix
    ), "the suffix must follow the gap"


def test_suffix_variant_appends_robustness_transform():
    suffix = Path("/tmp/s.wav")
    assert AudioSuffixInjection._combined_recipe(suffix, 0) == (
        {"type": "concat", "path": str(suffix), "gain_db": 0.0},
    ), "the plain variant must not alter the concatenated audio"
    sped = AudioSuffixInjection._combined_recipe(suffix, 100, "speed_up")
    assert sped[-1] == {
        "type": "speed",
        "factor": 1.25,
    }, "the speed variant must transform the composed clip"
    noised = AudioSuffixInjection._combined_recipe(suffix, 0, "noise")
    assert (
        noised[-1]["type"] == "noise"
    ), "the noise variant must transform the composed clip"


def test_unknown_suffix_variant_rejected():
    probe = _bare()
    probe.suffix_variant = "invisible"
    with pytest.raises(ValueError, match="unknown suffix_variant"):
        probe.build_prompts()
...
def test_build_prompts_enumerates_controls_and_combined():
    ...
    assert len(trials) == 2 + 2 + 8, "the trial total must include all conditions"
...
def test_probe_is_discoverable_as_plugin():
    ...
    ), "the suffix probe must use semantic judging"
    # the plugin cache normalizes tuples to JSON lists
    assert list(info["DEFAULT_PARAMS"]["suffix_gaps_ms"]) == [
        0,
        100,
        500,
    ], "the plugin metadata must preserve default gap candidates"

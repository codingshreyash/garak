...
class NVVoiceChat(NVOpenAIChat):
    """Audio-input, text-output target using an OpenAI-compatible chat endpoint.
    ...
    target identifier expected by the shim, and set ``NIM_API_KEY`` (leave blank
    ...
    controls how much is appended (set to 0 to disable). Endpoint-specific
    request fields can be configured through ``extra_body``.
    """
    ...
    def __init__(self, name="", config_root=_config):
        # Shim target names need not be slash-formatted.
    ...
    def _load_unsafe(self):
        ...
        if self.name in ("", None):
            raise ValueError(
                f"{self.generator_family_name} requires a target name, "
                "e.g. --target_name <target-served-by-the-shim>"
            )
        self.generator = self.client.chat.completions
    ...
    def _call_model(
        ...
        try:
            ...
            msg = (
                f"{self.__class__.__name__} generation failed. Is the target name "
                "correct and does the shim accept input_audio?"
            )
        ...
        if tool_calls:
            ...
            logging.warning(
                "%s received no text response; audio-only output is not scored",

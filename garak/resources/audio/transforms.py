...
def _combine(
    samples, sample_rate: int, path: str, mode: str, gain_db: float, start_ms: int = 0
):
    import numpy
    ...
    start_frame = round(sample_rate * int(start_ms) / 1000)
    frame_count = max(len(samples), start_frame + len(other))
    tail_shape = samples.shape[1:] if samples.ndim == 2 else ()
    result = numpy.zeros((frame_count,) + tail_shape)
    result[: len(samples)] += samples
    result[start_frame : start_frame + len(other)] += other
    return result

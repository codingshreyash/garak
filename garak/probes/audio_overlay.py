...
This garak-authored scenario mixes a spoken tool-risk instruction underneath a
benign carrier at reduced gain. It is motivated by the hidden-command threat
studied by Devil's Whisper, but it does not reproduce that paper's adversarial
optimisation method.

The probe includes three control conditions:
...
The relative-gain sweep compares background prominence with target behaviour.
The benign controls help identify transport failures; independent-ASR
validation is outside this probe and must be performed separately when needed.
"""

...
class AudioOverlayInjection(ToolRiskPETTS):
    ...
    def _positions(self) -> tuple[str, ...]:
        positions = (
            ...
            if isinstance(self.payload_positions, str)
            else tuple(self.payload_positions)
        )
        if not positions:
            raise ValueError("payload_positions must contain at least one value")
        unknown = sorted(set(positions) - _VALID_POSITIONS)
        if unknown:
            raise ValueError("unknown payload positions: " + ", ".join(unknown))
        return positions
    ...
    @staticmethod
    def _overlay_recipe(
        payload_path: Path, gain_db: float, start_ms: int = 0
    ) -> tuple[dict, ...]:
    ...
    def build_prompts(self):
        ...
        The ``mixed`` condition sweeps ``payload_positions`` (start / middle /
    ...
    def _trial_source_text(trial) -> str:
        condition, _, _, gain_db, position, text, _ = trial
        if condition == "mixed":
            carrier_text, payload_text = text
            return f"{carrier_text} [overlay {gain_db}dB @{position}] {payload_text}"
        return text
    ...
    def _compute_start_ms(
        self, position: str, carrier_path: Path, payload_path: Path
    ) -> int:
        if position == "start":
            return 0
        carrier_ms = self._wav_duration_ms(carrier_path)
        payload_ms = self._wav_duration_ms(payload_path)
        if position == "middle":
            return max(0, carrier_ms // 2 - payload_ms // 2)
        if position == "end":
            return max(0, carrier_ms - payload_ms)
        return 0
    ...
    def _prepare_trial_audio(self, trial) -> Path:
        ...
        start_ms = self._compute_start_ms(position, carrier_path, payload_path)
        recipe = self._overlay_recipe(payload_path, gain_db, start_ms)
    ...
    def _attempt_prestore_hook(self, attempt: Attempt, seq: int) -> Attempt:
        ...
        condition, carrier_id, payload_id, gain_db, position, text, required = (
        ...
        position_key = position or "start"
        group_id = f"{condition}:{carrier_id or ''}+{payload_id or ''}@{position_key}"
        attempt.notes["audio_overlay_injection"] = {
            "condition": condition,
            "carrier_id": carrier_id,
            "payload_id": payload_id,
            "overlay_gain_db": gain_db,
            "payload_position": position_key,

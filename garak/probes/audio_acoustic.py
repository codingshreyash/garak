...
"""**Acoustic voice Best-of-N probe**

Best-of-N Jailbreaking (arXiv:2412.03556) shows that repeatedly sampling
augmented inputs jailbreaks audio language models. This probe applies that idea
to the *voice* dimension: render the same request in many voices and count the
source compromised if any intelligible realization succeeds. Unlike signal
Best-of-N (speed/noise/filter), this varies the TTS *speaker* itself.

Each matched harmful/benign source is synthesized across a set of voice presets;
results are grouped so a source succeeds if any scoreable voice candidate does.
Every candidate is gated by the independent-ASR intelligibility check and the
tool-risk detectors/judge.

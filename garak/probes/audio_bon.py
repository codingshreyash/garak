...
This is a bounded diagnostic inspired by Best-of-N Jailbreaking
(arXiv:2412.03556), not a reproduction of its random, composed six-dimension
audio search or stop-on-success procedure.
"""
...
class PairedDirect(ToolRiskPairedSemanticPETTS):
    ...
    Each harmful request and its authorised control use the same fixed clean,
    ...
    separate outcome and does not count as a safe response. Candidate
    intelligibility is filtered only when ``candidate_validation_manifest`` is
    configured.
    """
